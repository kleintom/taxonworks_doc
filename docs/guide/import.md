...
| `catalogNumber`   | The identifier value for a Catalog Number local identifier. The namespace is selected from the namespaces lookup table in import settings queried by the `institutionCode`:`collectionCode` pair (or, alternatively, just the `collectionCode`). If you require several records to share the same Catalog Number identifier, you may do so by enabling the `Containerize specimen with existing ones when catalog number already exists` import setting or by distinct `recordNumber` values [TODO: does that work? i.e. are catalog identifiers required to be distinct?].            |
| `recordNumber`    | The identifier value for a Record Number local identifier. If not empty the record is required to have the short name of the Namespace to pair with the `recordNumber` in a separate TW-specific column named `TW:Namespace:RecordNumber`. This DwC term enables the re-use of the same `catalogNumber` of both existing collection objects and records in the dataset, as the importer assigns related specimens to a container to allow sharing the same Catalog Number identifier. <-- TODO unravel this |
| `recordedBy`      | Imported as-is into the verbatim collectors field of the collecting event. Additionally, the value is parsed into people and assigned as collectors of the Collecting Event. Previously existing people are not used unless the data origin is the same dataset the record belongs to, otherwise any missing people are created. <-- TODO: unravel this                                                                                                                             |
...
| `preparations`    | Selects an existing preparation having the same name. TODO: are preparations that don't match ignored or errors?                                                                                                                                                                                                                                                                                                                                                                              |

##### Event class

| Term                | Mapping                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `eventID`           | The identifier for the Collecting Event. If not empty the importer requires a Namespace for it. Option 1: You may specify that Namespace in a TW-specific column named `TW:Namespace:EventID` with either a global identifier type (e.g. `Identifier::Global::Uuid`, `Identifier::Global::Lsid`, etc.), or the short name of a TW namespace. Option 2: If no namespace is provided, the importer assigns a dataset-specific one with a synthetic name that you can later change. _When an existing TW Collecting Event already has the identifier you define in this way, the importer re-uses it and all other event-related data is ignored_.  TODO: if this is local then it's a field # - ah, read the end of the `fieldNumber` description, that probably needs to be repeated here.                  |
| `fieldNumber`       | The identifier value for a Field Number local identifier. If not empty then the short name of the Namespace to use with the identifier is required to be in a TW-specific column named `TW:Namespace:FieldNumber`. TODO: this no longer exists, right? --> The verbatim trip identifier is also populated by this DwC term. _When an existing Collecting Event already has this identifier, the importer re-uses it and all other event-related data is ignored._ IMPORTANT: if a Collecting Event is already matched by `eventID`, this identifier must exactly match the existing one, otherwise the importer will reject the record. The same rejection will occur if a mismatch happens the other way around. |
...
##### Location class <-- TODO: is this still used? I think trip identifier is out
...
| Term             | Mapping                                                                                                                                                                                                                                                                            |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `identifiedBy`   | A list of names of people, groups, or organizations who assigned the Taxon to the subject. If possible, separate the values in a list with a vertical bar \| (known as a pipe) with a space on either side (e.g. <code>Theodore Pappenfuss &#124; Robert Macey</code>) TODO: what do these map to?|
| `dateIdentified` | The date on which the subject was determined as representing the Taxon. Best practice is to use a date that conforms to ISO 8601-1:2019 [see examples](https://dwc.tdwg.org/terms/#dwc:dateIdentified). TODO: how to do a list (is it possible?) TODO: what do these map to?                                                                            |

##### Taxon class

| Term                       | Mapping                                                                                                                                                                                                                                                                                                                                                                               |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `nomenclaturalCode`        | Selects the nomenclatural code for the taxon rank used when creating protonyms - allowed values are `iczn`, `icn`, `icnp`, `icvcn` (case-insensitive). The value itself is not imported. If Taxon Names need to be created then either this field or the corresponding default setting must be provided (this field overrides the default when both exist). <-- TODO: check this last sentence                                                                                                                                                                                                                                                                     |
...
| `scientificNameAuthorship` | Verbatim author of most specific protonym. TODO: not possible to associate with an existing source I guess? match task?                                                                                                                                                                                                                                                                                                                                           |
